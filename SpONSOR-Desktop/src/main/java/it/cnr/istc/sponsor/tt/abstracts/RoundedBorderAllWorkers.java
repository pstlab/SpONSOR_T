 ...
 */
public class RoundedBorderAllWorkers extends AbstractBorder {
    ...
    public RoundedBorderAllWorkers(Color c, int g) {
        color = c;
        gap = g;
    }
...
//        g2d.setColor(personAvailable >= 0 ? Color.GREEN : Color.RED);
        g2d.setColor(personAvailable >= 0 ? Color.RED : Color.RED);
        if (personAvailable <= 0) {
            g2d.drawImage(icon.getImage(), (int) ((width - 32) / 2), 0,null);
        } else {
            int stringWidth = g2d.getFontMetrics().stringWidth("" + personAvailable);
            int stringHeight = g2d.getFontMetrics().getHeight();
            g2d.drawString("" + personAvailable, (int) ((width - stringWidth) / 2), height - 7); //(int)((height-stringHeight)/2)
        }
        g2d.dispose();
    }
